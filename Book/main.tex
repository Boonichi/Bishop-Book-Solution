...
\begin{document}
    ...
    *Note: Incorrect, the theta variable called kronecker delta and the derivative of equation is incorrect
    ...
    This Problem is solved like problem 1.10.
    ...
    \begin{align*}
        \intertext{The proof of the equation (2.62) is in book so we don't prove it. Consequently, we will prove the equation (2.124)}
    ...
    The solution of $w_{ML}$ is metioned in the equation (3.12) and (3.1) if we subtitute $\beta^{-1} = r_n\beta^{-1}$. The derivative of the equation (3.104) by w:
    \begin{align*}
        \frac{\partial}{\partial w} E_D(w) = \frac{\partial}{\partial w}\sum_{n = 1}^{N}r_n(t_n + w^T\phi(x_n)) \\
        = \sum_{n = 1}^{N}r_n(t_n + w^T\phi(x_n))\phi(x_n)
    \end{align*}
    Set the derivative of the equation equal zero and based on the equation (3.16), we obtain:
    \begin{align*}
        w_ML = r_n(\Phi^T\Phi)^{-1}\Phi^Tt
    \end{align*}
\end{document}